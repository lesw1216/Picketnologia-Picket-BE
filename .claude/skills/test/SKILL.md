...
```
대상: src/main/java/com/picketlogia/picket/api/product/service/ProductService.java
생성: src/test/java/com/picketlogia/picket/api/product/service/ProductServiceTest.java
```
...
- 정상 케이스 (happy path)
- 존재하지 않는 ID / 데이터 없음 → CustomException 발생
- 유효하지 않은 상태 전이 → CustomException 발생 (해당되는 경우)
